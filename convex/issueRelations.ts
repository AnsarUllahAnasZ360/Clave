...
async function canReadIssue(
	...
	return issue.assigneeId === userId || issue.createdBy === userId;
}
...
async function requireIssueAccess(
	...
	const hasAccess = await canReadIssue(ctx, issue, userId, memberRole);
	if (!hasAccess) {
		throw new ConvexError("You don't have access to this issue");
	}
}
...
export const listByIssue = query({
	...
	handler: async (ctx, args) => {
		...
		if (!(await canReadIssue(ctx, issue, userId, role))) {
			return { blocks: [], blocked_by: [], relates_to: [], duplicate: [] };
		}
		...
		for (const rel of relations) {
			...
			if (!(await canReadIssue(ctx, otherIssue, userId, role))) continue;
...
export const getBlockers = query({
	...
	handler: async (ctx, args) => {
		...
		const role = member.role as WorkspaceRole;
		if (!(await canReadIssue(ctx, issue, userId, role))) return [];
		...
		for (const rel of blockedByRelations) {
			...
			if (
				blocker &&
				!blocker.deletedAt &&
				(await canReadIssue(ctx, blocker, userId, role))
			) {
				blockers.push({
					_id: blocker._id,
...
/** Given issue IDs, return the subset that are blocked by at least one other issue. */
export const blockingIssueIdsForIssues = query({
	args: {
	...
	handler: async (ctx, args) => {
		...
		if (!firstIssue || firstIssue.deletedAt) return [];
		const { userId, member } = await requireWorkspaceMember(
			ctx,
		...
		const blocked: Id<"issues">[] = [];
		for (const issueId of args.issueIds) {
			const issue = await ctx.db.get(issueId);
			if (!issue || issue.deletedAt) continue;
			if (issue.workspaceId !== firstIssue.workspaceId) continue;
			if (!(await canReadIssue(ctx, issue, userId, role))) continue;

			const blockers = await ctx.db
				.query("issueRelations")
				.withIndex("by_issue_type", (q) =>
					q.eq("issueId", issueId).eq("type", "blocked_by"),
				)
				.take(1);

			if (blockers.length > 0) {
				blocked.push(issueId);
			}
		}
...
export const create = mutation({
	...
	handler: async (ctx, args) => {
		...
		const role = member.role as WorkspaceRole;
		await requireIssueAccess(ctx, issue, userId, role);
		await requireIssueAccess(ctx, relatedIssue, userId, role);
