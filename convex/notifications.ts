...
export const list = query({
	...
	handler: async (ctx, args) => {
		...
		// Fetch more to account for filtering; cap at 500 notifications
		const raw = await buildQuery().take(Math.min((limit + 1) * 2, 500));
...
export const listSnoozed = query({
	...
	handler: async (ctx, args) => {
		...
		// Filter to only snoozed (snoozedUntil in the future) and not deleted
		const snoozed = raw
			.filter(
				(n) =>
					!n.deletedAt && n.snoozedUntil !== undefined && n.snoozedUntil > now,
			)
			.slice(0, limit);
...
export const unsnoozeExpired = internalMutation({
	handler: async (ctx) => {
		...
		// We scan recent notifications and check snoozedUntil in memory
		// since we don't have an index on snoozedUntil
		const candidates = await ctx.db.query("notifications").take(5000);
	...
	handler: async (ctx) => {
		...
		for (const workspace of workspaces) {
			...
			for (const issue of issues) {
				...
				if (issue.dueDate > now && issue.dueDate <= in24Hours) {
		...
		if (count > 0) {
			console.log(`Sent ${count} due-date reminder notifications`);
		}
	},
});
