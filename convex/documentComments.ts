...
import { requireWorkspaceMember, tryWorkspaceMember } from "./lib/auth";
...
/** Check if body contains an @AI mention node (works with both TipTap and BlockNote JSON) */
function hasAIMention(bodyJson: string): boolean {
	try {
		const body = JSON.parse(bodyJson);
		function walk(node: Record<string, unknown>): boolean {
			if (
				node.type === "mention" &&
				(node.attrs as Record<string, unknown>)?.entityType === "ai"
			) {
				return true;
			}
			const content = node.content as Record<string, unknown>[] | undefined;
...
export const listThreadsByDocument = query({
	...
	handler: async (ctx, args) => {
		...
		const membership = await tryWorkspaceMember(ctx, document.workspaceId);
...
export const createThread = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (hasAIMention(args.initialCommentBody)) {
			await ctx.scheduler.runAfter(
				0,
				internal.ai.embedded.handleDocumentAIMention,
				{
					threadId,
					documentId: args.documentId,
					workspaceId: document.workspaceId,
					commentBody: args.initialCommentBody,
				},
			);
		}
		...
		if (!thread || !comment) {
			throw new ConvexError("Failed to create comment thread");
		}
