...
/** Issues for a specific milestone, ordered by sortOrder */
...
export const create = mutation({
	...
	handler: async (ctx, args) => {
		...
		// Compute sortOrder: append at end of project or milestone
...
export const bulkUpdateStatus = mutation({
	...
	handler: async (ctx, args) => {
		...
		for (const issueId of args.issueIds) {
			...
			// RBAC: verify access for each issue
...
export const createSubIssue = mutation({
	...
	handler: async (ctx, args) => {
		...
		// Inherit projectId and milestoneId from parent if not explicitly provided
		const projectId = args.projectId ?? parent.projectId;
...
export const myIssuesActivity = query({
	...
	handler: async (ctx, args) => {
		...
		// Get recent activity where user was the actor
		...
		// Collect unique issue IDs from activity involving this user
		const issueIdSet = new Set<string>();
		for (const log of actorLogs) {
			if (log.issueId && log.actorId === userId) {
				issueIdSet.add(log.issueId);
			}
		}

		const issueResults = await Promise.all(
			Array.from(issueIdSet).map(async (id) => {
