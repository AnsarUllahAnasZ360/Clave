...
const KEY_HINTS: Array<{ pattern: RegExp; category: StatusCategory }> = [
	{ pattern: /^(triage|inbox|new|incoming|needs[_-]?triage)$/i, category: "backlog" },
	{ pattern: /^(backlog|icebox|someday|later|ideas?)$/i, category: "backlog" },
	{ pattern: /^(todo|to[_-]?do|ready|next|planned|up[_-]?next)$/i, category: "unstarted" },
	{ pattern: /^(in[_-]?progress|doing|started|active|wip|working|started)$/i, category: "started" },
	{ pattern: /^(in[_-]?review|review|qa|testing?|verify|verification|staging|uat|on[_-]?hold|blocked|paused|waiting)$/i, category: "started" },
	{ pattern: /^(done|complete|completed|shipped|deployed|released|finished|closed|merged)$/i, category: "completed" },
	{ pattern: /^(cancel(l)?ed|wontfix|won['_-]?t[_-]?fix|abandoned|rejected|duplicate|invalid|archived)$/i, category: "canceled" },
];
...
const NAME_KEYWORDS: Array<{ pattern: RegExp; category: StatusCategory }> = [
	...
	{ pattern: /\b(progress|doing|wip|working|active|started)\b/i, category: "started" },
	{ pattern: /\b(review|qa|testing|test|staging|verif|uat|hold|block|paused|waiting)\b/i, category: "started" },
	{ pattern: /\b(done|complet|shipped|deployed|released|finished|closed|merged)\b/i, category: "completed" },
	{ pattern: /\b(cancel|won.?t|abandon|reject|duplicate|invalid|archiv)\b/i, category: "canceled" },
];
