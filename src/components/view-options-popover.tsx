	...
	return (
		<Popover>
			...
			<PopoverContent className="w-80 rounded-xl p-0" align="end">
				<div className="p-4">
					{/* View Type Tabs -- hidden when only one view type is allowed */}
					{viewTypes.length > 1 && (
							...
							{viewTypes.map((type) => (
								<button
									...
									onClick={() =>
										onChange({
											...options,
											viewType: type.id as Options["viewType"],
										})
									}
									className={cn(
					...
					<div className={cn(viewTypes.length > 1 ? "mt-4" : "", "space-y-3")}>
							...
							<Popover open={orderingOpen} onOpenChange={setOrderingOpen}>
								<PopoverTrigger asChild>
									...
									>
										<Sliders className="h-4 w-4" />
										{
											orderingOptions.find((o) => o.id === options.ordering)
												?.label
										}
										<CaretUpDown className="h-3.5 w-3.5 text-muted-foreground" />
									</Button>
									...
									{orderingOptions.map((option) => (
										<button
											...
											onClick={() => {
												onChange({
													...options,
													ordering: option.id as Options["ordering"],
												});
						...
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									onChange({ ...options, showClosedProjects: checked })
								}
							/>
						</div>
							...
							<Popover open={groupByOpen} onOpenChange={setGroupByOpen}>
								<PopoverTrigger asChild>
									...
									>
										...
										{
											groupByOptions.find((o) => o.id === options.groupBy)
												?.label
										}
									</Button>
								</PopoverTrigger>
									...
									{groupByOptions.map((option) => (
										<button
											...
											onClick={() => {
												onChange({
													...options,
													groupBy: option.id as Options["groupBy"],
												});
