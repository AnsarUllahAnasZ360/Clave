...
export function TaskWeekBoardView({
	...
	const getDayKey = useCallback((date: Date) => format(date, "yyyy-MM-dd"), []);
	...
	const dayKeys = useMemo(
		() => weekColumns.map((c) => getDayKey(c.date)),
		[weekColumns, getDayKey],
	);

	const [activeTask, setActiveTask] = useState<ProjectTask | undefined>(
		undefined,
	);
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		const targetColumn = weekColumns.find(
			(col) => getDayKey(col.date) === overDay,
		);
		...
		setColumnOrder((prev) => {
			...
			const nextTarget = target.includes(activeId)
				? target
				: [...target, activeId];
				...
				<div className="overflow-x-auto flex-1 min-h-0">
					<div className="flex gap-4 px-4 pb-4 min-w-max">
						{weekColumns.map((column) => {
							...
							const orderedIds =
								columnOrder[dayKey] ?? column.tasks.map((t) => t.id);
