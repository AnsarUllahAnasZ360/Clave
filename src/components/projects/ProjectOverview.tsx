...
> = {
	...
	active: { label: "Active", icon: CircleDot, color: "text-yellow-500" },
	completed: {
		label: "Completed",
		icon: CircleCheck,
...
export function ProjectOverview({
	...
	const milestones = useQuery(api.milestones.listByProject, {
	...
	const statusCfg = STATUS_DISPLAY[project.status];
	const StatusIcon = statusCfg.icon;

	return (
		<div className="space-y-6 py-4">
			{/* Project identity: icon + name + summary */}
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				<div className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-1 text-xs font-medium">
					...
					<span className="text-foreground">{statusCfg.label}</span>
				</div>
				...
				<div className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-1 text-xs font-medium">
					...
					<span className="text-foreground">
						{PRIORITY_DISPLAY[project.priority] ?? project.priority}
					</span>
				</div>
				...
				{lead?.user?.name && (
					<div className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-1 text-xs font-medium">
						...
						<span className="text-foreground">{lead.user.name}</span>
					</div>
				)}
...
function MilestonesSection({
	...
	const reorderMilestone = useMutation(api.milestones.reorder);
...
function SortableMilestoneRow({
	...
	onOpenDetail: (id: Id<"milestones">) => void;
}) {
	const {
		attributes,
		listeners,
	...
	const handleComplete = useCallback(async () => {
		try {
			if (isCompleted) {
				...
				await completeMilestone({ milestoneId: milestone._id });
			}
		} catch {
