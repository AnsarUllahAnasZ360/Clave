...
export function Comment(props: {
	...
	const resolveDiscussion = async (id: string) => {
		...
		// Persist to backend
		await callbacks?.onResolveDiscussion?.({ discussionId: id });
	};
	...
	const removeDiscussion = async (id: string) => {
		...
		editor.setOption(discussionPlugin, "discussions", updatedDiscussions);
		// Persist to backend
		await callbacks?.onDeleteDiscussion?.({ discussionId: id });
	};
...
function CommentMoreDropdown(props: {
	...
	const onDeleteComment = React.useCallback(() => {
		...
		// Optimistic update: remove comment from discussion
		const updatedDiscussions = editor
			.getOption(discussionPlugin, "discussions")
			.map((discussion) => {
		...
		// Persist to backend
		void callbacks?.onDeleteComment?.({
			discussionId: comment.discussionId,
...
export function CommentCreateForm({
	...
	const onAddComment = React.useCallback(async () => {
		...
		if (discussionId) {
			...
			if (!discussion) {
				// Creating new discussion for an existing comment mark
		...
		// Creating a brand new discussion from draft marks
		...
		// Convert draft marks to permanent comment marks
