...
export function EmojiToolbarButton({
	...
	// Emoji data is loaded lazily. Guard against rendering the picker
	// before data arrives (useEmojiDropdownMenuState requires it).
	const data = usePluginOption(EmojiPlugin, "data");

	if (!data) {
		return (
			<ToolbarButton tooltip="Emoji" isDropdown disabled {...props}>
				<SmileIcon />
			</ToolbarButton>
		);
	}

	return <EmojiToolbarButtonReady options={options} {...props} />;
}

function EmojiToolbarButtonReady({
	options,
	...props
}: {
	options?: EmojiDropdownMenuOptions;
