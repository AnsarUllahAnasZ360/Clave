...
export const MentionList = forwardRef<MentionListRef, MentionListProps>(
	({ items, command }, ref) => {
		...
		const aiItems = items.filter(
			(i): i is MentionItem & { type: "ai" } => i.type === "ai",
		);
					...
					<div>
						...
						{aiItems.map((item) => {
							...
							return (
								...
								>
									<SparklesIcon className="h-4 w-4 shrink-0 text-sienna-500 dark:text-sienna-400" />
									<span className="truncate text-sm font-medium text-sienna-600 dark:text-sienna-400">
										{item.data.name}
									</span>
								</MentionOption>
							);
