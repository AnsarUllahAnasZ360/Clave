	...
	return [
		...
		CustomMention.configure({
			...
			renderHTML({ options, node }) {
				...
				// AI mention — non-clickable, styled via CSS
				if (entityType === "ai") {
					return [
						"span",
						mergeAttributes(options.HTMLAttributes, attrs),
						`@${node.attrs.label ?? "Clave AI"}`,
					];
				}
...
/**
 * Check if body is a JSON array (BlockNote CommentBody or Slate Value).
 * e.g. [{"type":"text","text":"hello","styles":{}}]          — BlockNote
 * e.g. [{"type":"p","children":[{"text":"hello"}]}]          — Slate Value
 */
function isJsonArrayBody(body: string): boolean {
	if (!body.startsWith("[")) return false;
	try {
		const parsed = JSON.parse(body);
...
/**
 * Extract plain text from a JSON array body (BlockNote CommentBody or Slate Value).
 * Walks both `content` (BlockNote) and `children` (Slate) recursively.
 */
function extractJsonArrayText(body: string, maxLength = 100): string {
	try {
		const nodes = JSON.parse(body) as Array<Record<string, unknown>>;
		const texts: string[] = [];
		...
		function walk(node: Record<string, unknown>) {
			...
			// Slate mention nodes: { type: "mention", value: "Name", children: [{text:""}] }
...
export function CommentContent({
	...
	// Handle JSON array format (BlockNote CommentBody or Slate Value)
	if (isJsonArrayBody(body)) {
		const text = extractJsonArrayText(body);
		return (
			<div className={className}>
...
export function extractCommentText(body: string, maxLength = 100): string {
	...
	// Handle JSON array format (BlockNote CommentBody or Slate Value)
	if (isJsonArrayBody(body)) {
		return extractJsonArrayText(body, maxLength);
	}
