...
export function ClientDetailsDrawer({
	...
	const relatedProjects = useQuery(
		api.clients.getProjects,
		clientId ? { clientId: clientId as Id<"clients"> } : "skip",
	);

	if (!clientId) return null;

	if (client === undefined || relatedProjects === undefined) {
		return (
	...
	const totalPages = Math.max(
		1,
		Math.ceil((relatedProjects?.length ?? 0) / projectsPerPage),
	);
		...
		<div className="fixed inset-0 z-40 flex justify-end">
			...
			<aside className="flex h-full w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl">
					...
					<div className="flex items-start justify-between gap-3">
						...
						<div className="flex items-center gap-2">
							<div className="hidden sm:block">
								<ClientStatusFilterPopover
									initialStatus={client.status as "prospect" | "active" | "on_hold" | "completed" | "archived"}
									clientId={client._id}
								/>
							</div>
							<Button
				...
				{isWizardOpen && (
					<ClientWizard
						mode="edit"
						initialClient={{ ...client, status: client.status as "prospect" | "active" | "on_hold" | "completed" | "archived" }}
