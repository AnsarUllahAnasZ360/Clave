...
export function PropertyShortcutPicker() {
	...
	const handleMilestoneSelect = useCallback(
		(milestoneId: string) => {
			...
			const ms = milestones?.find((m) => m._id === milestoneId);
			toast.success(`Sprint set to ${ms?.name ?? "sprint"}`);
		},
		[activeIssueId, updateMut, milestones],
	);
	...
	> = {
		...
		milestone: {
			...
			currentValue: issue?.milestoneId ?? undefined,
