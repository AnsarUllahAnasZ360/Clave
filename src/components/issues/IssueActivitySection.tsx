...
function CommentItem({
	...
	onAIReply?: (commentId: Id<"comments">, text: string) => void;
	submitting: boolean;
	isReply?: boolean;
	...
	const isAIAuthored = comment.author.name === "Clave AI";
					...
					<div className="flex items-center gap-2">
						...
						{!isEditing && (
							<div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-0.5">
								...
								{!isAIAuthored && onAIReply && workspaceId && issueId && (
									<AICommentReply
										workspaceId={workspaceId}
										issueId={issueId}
										commentBody={extractCommentText(comment.body, 500)}
										commentAuthor={comment.author.name}
										onReplyGenerated={(text) => onAIReply(comment._id, text)}
									/>
								)}
								<DropdownMenu>
		...
		<div>
			...
			{replyingTo === comment._id && (
				<div className="ml-6 mt-1 pl-2">
					<CommentEditor
						key={aiReplyContent ? "ai-reply" : "manual-reply"}
						initialContent={aiReplyContent || undefined}
...
export function IssueActivitySection({ issueId }: { issueId: Id<"issues"> }) {
	...
	const [submitting, setSubmitting] = useState(false);
	const [aiReplyContent, setAiReplyContent] = useState<string | null>(null);
	...
	const handleAIReply = useCallback(
		(commentId: Id<"comments">, text: string) => {
			setReplyingTo(commentId);
			setEditingId(null);
			setAiReplyContent(text);
		},
		[],
	);
