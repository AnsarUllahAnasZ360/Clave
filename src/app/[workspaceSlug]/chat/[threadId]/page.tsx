...
export default function ChatThreadPage() {
	...
	const handleOpenThread = useCallback(
		(openThreadId: string) => {
			...
			router.push(`/${workspaceSlug}/chat/${openThreadId}` as Route);
		},
		...
		async (deleteThreadId: string) => {
			...
			if (deleteThreadId === threadId) {
				...
				if (remaining.length > 0) {
					router.replace(
						`/${workspaceSlug}/chat/${remaining[0].threadId}` as Route,
					);
				} else {
					router.replace(`/${workspaceSlug}/chat` as Route);
				}
			}
		},
		[chat.deleteThread, chat.threads, threadId, workspaceSlug, router],
	);
