/**
 * Vendored from official Excalidraw MCP server:
 * https://github.com/excalidraw/excalidraw-mcp
 * Upstream commit: 542091bff3517f965b67b77dd5af5566817f682d
 *
 * Keep behavior/tool contracts in parity with upstream unless explicitly noted.
 */
/* biome-ignore-all lint/suspicious/noExplicitAny: upstream parity file */
...
// In this app we ship the built widget HTML alongside the vendored server.
	...
	registerAppTool(
		...
		async ({ json }): Promise<CallToolResult> => {
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: `Export failed: ${(err as Error).message}` },
					],
					isError: true,
				};
			}
	...
	registerAppTool(
		...
		async ({ id, data }): Promise<CallToolResult> => {
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: `save failed: ${(err as Error).message}` },
					],
					isError: true,
				};
			}
	...
	registerAppTool(
		...
		async ({ id }): Promise<CallToolResult> => {
			try {
				...
				return {
					content: [
						{ type: "text", text: `read failed: ${(err as Error).message}` },
					],
					isError: true,
				};
			}
