...
describe("AI model defaults", () => {
	it("uses GPT 5.2 as the default model across frontend and backend", () => {
		expect(DEFAULT_MODEL_ID).toBe("gpt-5.2");
		expect(DEFAULT_CHAT_MODEL_ID).toBe("gpt-5.2");
	});

	it("normalizes missing and unknown model ids to GPT 5.2", () => {
		expect(normalizeChatModelId()).toBe("gpt-5.2");
		expect(normalizeChatModelId("unknown-model")).toBe("gpt-5.2");
	});
