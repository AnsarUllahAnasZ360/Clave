...
test.describe("Knowledge Tab", () => {
	...
	test("Knowledge tab shows filter chips", async ({ page }) => {
		...
		await expect(
			page.locator("button").filter({ hasText: /Docs/i }).first(),
		).toBeVisible();
		await expect(
			page
				.locator("button")
	...
	test("filter chips filter the knowledge list", async ({ page }) => {
		...
		const docsFilter = page.getByRole("button", { name: /Docs/i });
		...
		// Click "Boards" filter
		const boardsFilter = page.getByRole("button", { name: /Boards/i });
