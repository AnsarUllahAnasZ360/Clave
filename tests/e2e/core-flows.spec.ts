...
test.describe("Landing page", () => {
	...
	test("landing page renders content or loading state", async ({
		unauthenticatedPage: page,
	}) => {
		await page.goto("/");
		await waitForHydration(page);

		// The landing page shows either "Clave" heading (when Convex returns
		// user === null) or "Loading..." (when Convex is not connected).
		// Both are valid states depending on backend availability.
		await expect(
			page
				.getByRole("heading", { name: "Clave" })
...
test.describe("Responsive design", () => {
	test("landing page loads correctly on mobile viewport", async ({
		...
		// Should render either landing content or loading state
		await expect(
			page
				.getByRole("heading", { name: "Clave" })
				.or(page.getByText("Loading...")),
